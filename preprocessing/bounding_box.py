...
def bbox_auto(sitk_pt, pID, output_shape=(128, 128, 128), th=3):
    ...
    if np.int32((y2 + y1) / 2 - yshift - np.int32(output_shape_pt[1] / 2)) < 0:
        ybb = (0, output_shape_pt[1])
    elif np.int32((y2 + y1) / 2 - yshift -
                np.int32(output_shape_pt[1] / 2)) > np_pt.shape[1]:
    ...
    if np.int32((x2 + x1) / 2 - np.int32(output_shape_pt[0] / 2)) < 0:
        xbb = (0, output_shape_pt[0])
    elif np.int32((x2 + x1) / 2 -
                np.int32(output_shape_pt[0] / 2)) > np_pt.shape[0]:
...
# testing set
patient_info_path = '/media/sergio/TOSHIBA EXT1/Master/TFM/HECKTOR/test_2/hecktor2022_testing_v2/hecktor2022_testing/hecktor2022_clinical_info_testing.csv'
pet_path = '/media/sergio/TOSHIBA EXT1/Master/TFM/HECKTOR/test_2/hecktor2022_testing_v2/hecktor2022_testing/imagesTs/'
bb_save_path = '/media/sergio/TOSHIBA EXT1/Master/TFM/HECKTOR/test_2/hecktor2022_testing_v2/hecktor2022_testing/bbox_2/bb_box_testing.csv'
...
for pID in patients_list:
    print(f"Generating bbox for patient: {pID} - {i}/{len(patients_list)}")
    bb = bbox_auto(sitk.ReadImage(pet_path + str(pID) + '__PT.nii.gz'), pID)
    bb_data.append({
        'PatientID': pID,
        'x1': bb[0],
...
# bb_df = pd.concat([pd.DataFrame(data) for data in bb_data], ignore_index=True)
bb_df = pd.concat([pd.DataFrame(data, index=[0]) for data in bb_data], ignore_index=True)
